...
VestraPay API is a comprehensive NestJS application designed for scalable, maintainable, and enterprise-grade development. It is built with a strong focus on the `repository design pattern` and a fully `modular architecture`. By leveraging these patterns, the codebase achieves clear separation of concerns, high testability, and easy extensibility for new features. 
...
The App Module is the root module and entry point for the VestraPay API application. It orchestrates the core setup by:
...
The Common Module provides shared functionality and global services across the VestraPay API application. It configures:
